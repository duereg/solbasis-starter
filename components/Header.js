export default function Header() {
  //return <h1 className="title">{title}</h1>
  return (
  <header>
    <div id="overlay">
        <ul className="text-center list-none text-white text-3xl">
            <li className="p-2">
                <a href="/contact">
                    <span>Contact</span>
                </a>
            </li>
            <li className="p-2">
                <a href="/about">
                    <span>About</span>
                </a>
            </li>
        </ul>
    </div>
    <nav className="flex items-center justify-between flex-wrap bg-opacity-100 p-8">
        <div className="flex items-center flex-shrink-0 text-xl text-grey-900 mr-6">
            <a href="https://www.solcapture.com">
              <img alt="SolCapture" className="h-14 mr-3" src="/img/logo/0x0.png" />
            </a>
        </div>
        <div className="block lg:hidden">
            <div id="hamburgerbtn" className="flex cursor-pointer items-center w-14 h-10 py-2 rounded mopen">
                <span className="top"></span>
                <span className="middle"></span>
                <span className="bottom"></span>
            </div>
        </div>
        <div className="hidden w-full block flex-grow lg:flex lg:items-center lg:w-auto" id="mobileMenu">
            <div className="text-lg lg:flex-grow">
            </div>
            <div className="text-lg">
                <a href="/contact" className="duration-200 block mt-4 lg:inline-block lg:mt-0 text-black hover:opacity-60 mr-8">
                    Contact
                </a>
                <a href="/about" className="duration-200 block mt-4 lg:inline-block lg:mt-0 text-black hover:opacity-60 mr-8">
                    About
                </a>
            </div>
        </div>
    </nav>
  </header>);
}
